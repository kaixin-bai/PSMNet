...
def dataloader(filepath):
    ...
    monkaa_path = os.path.join(filepath, [x for x in image if 'monkaa' in x][0])
    monkaa_disp = os.path.join(filepath, [x for x in disp if 'monkaa' in x][0])
    ...
    flying_path = os.path.join(filepath, [x for x in image if x == 'frames_cleanpass'][0])
    flying_disp = os.path.join(filepath, [x for x in disp if x == 'frames_disparity'][0])
    ...
    driving_dir = os.path.join(filepath, [x for x in image if 'driving' in x][0])
    driving_disp = os.path.join(filepath, [x for x in disp if 'driving' in x][0])
